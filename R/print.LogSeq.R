...
print.LogSeq<-function(x, ...){
  ...
  if(dim(x[[1]])[1]==1){
    ...
    if(length(x[[3]])==1){
      ...
      actor<-t.test(lambdas[,2])
      partner<-t.test(lambdas[,3])
    ...
    if(length(x[[3]])>1){
      ...
      actor.x<-t.test(lambdas.x[,2])
      partner.x<-t.test(lambdas.x[,3])
      interac.x<-t.test(lambdas.x[,4])

      intercept.y<-t.test(lambdas.y[,1])
      actor.y<-t.test(lambdas.y[,2])
      partner.y<-t.test(lambdas.y[,3])
      interac.y<-t.test(lambdas.y[,4])

      intercept.yx<-t.test(lambdas.y[,1],lambdas.x[,1])
      actor.yx<-t.test(lambdas.y[,2],lambdas.x[,2])
      partner.yx<-t.test(lambdas.y[,3],lambdas.x[,3])
